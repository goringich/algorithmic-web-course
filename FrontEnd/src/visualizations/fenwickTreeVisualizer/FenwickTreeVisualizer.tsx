...
import FenwickControls from './FenwickControls';
import FenwickCanvas from './FenwickCanvas';
import FenwickInfo from './FenwickInfo';
import { createFenwickTree } from '../dataStructures/fenwickTree';
...
function calculateNodePositions(size) {
  ...
  for (let level = 0; level < levels; level++) {
    ...
    for (let i = intervalSize; i <= size; i += intervalSize * 2) {
      positions[i] = { level, intervalSize, indexInLevel: nodesAtLevel, totalInLevel };
      nodesAtLevel++;
    }
  }
  return positions;
...
function calculateAbsolutePositions(relativePositions, width, height) {
  ...
  const levels = Math.max(...Object.values(relativePositions).map((p) => p.level)) + 1;
...
export default function FenwickTreeVisualizer() {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (containerRef.current) {
        ...
        setCanvasSize({ width, height: height * 0.95 });
        updatePositions(width, height * 0.95);
      }
    };
  ...
  function onArrayChange(val) {
    // just set local, apply later
  }

  function onApplyArray() {
    try {
      const parsed = JSON.parse(document.querySelector('input[label="Array Input (Ввод массива)"]').value);
  ...
  function handleRangeSum() {
    if (!bit || isAnimating) return;
    if (queryLeft < 1 || queryRight > array.length || queryLeft > queryRight) return;
    setIsAnimating(true);
  ...
  return (
    <Box>
      ...
      <Box display="flex" gap={2}>
        ...
        <Paper ref={containerRef} sx={{ flex: 2, minHeight: 500, position: 'relative' }}>
          {bit && (
            <FenwickCanvas
              bit={bit}
