...
export const updateTreeWithNewData = async (
  ...
  if (!currentLayer) {
    console.error("[ERROR] layerRef.current is null. Aborting (отмена) updateTreeWithNewData.");
    return null;
  }
  if (!segmentTree) {
    console.error("[ERROR] SegmentTreeWasm instance is not initialized (инициализирован).");
    return null;
  }
  try {
    await segmentTree.setData(newData);
    console.log("[INFO] segmentTree.setData completed (завершено).");
    let newVisNodes = await segmentTree.getTreeForVisualization();
    console.log("[INFO] segmentTree.getTreeForVisualization returned (возвращено):", newVisNodes);
    newVisNodes = normalizeVisNodes(newVisNodes);
    const rootId = newVisNodes[0]?.id;
    if (rootId === undefined) {
      throw new Error("No root node found in the new visualization (визуализация) nodes.");
    }
